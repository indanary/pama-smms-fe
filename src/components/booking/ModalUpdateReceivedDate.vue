      ...
      <q-card-section style="max-height: 60vh" class="scroll">
        <q-form greedy ref="formRef" style="display: flex; flex-direction: column; gap: 20px">
          <div style="display: flex; flex-direction: column; gap: 8px">
            <span class="app-input-required">Received Date</span>
            <q-input
              v-model="formState.received_date"
              type="text"
              filled
              dense
              outlined
              placeholder="Choose date"
              :mask="formState.received_date"
            ...
            >
              ...
              <q-popup-proxy transition-show="scale" transition-hide="scale">
                <q-date minimal v-model="formState.received_date" mask="YYYY-MM-DD">
    ...
    onUpdate(): void {
      ;(this.$refs.formRef as QForm).validate().then((isValid) => {
        ...
        const payload: PayloadUpdateBooking = {
          received_date: this.formState.received_date,
        }
